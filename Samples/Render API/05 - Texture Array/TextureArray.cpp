...
void TextureArray::OnUpdate()
{
	...
	if (m_Keyboard.KeyWentDown(BvKey::k0))
	{
		m_CurrArrayIndex = 0;
	}
	else if (m_Keyboard.KeyWentDown(BvKey::k1))
	{
		m_CurrArrayIndex = 1;
	}
	else if (m_Keyboard.KeyWentDown(BvKey::k2))
	{
		m_CurrArrayIndex = 2;
	}

	if (m_Keyboard.KeyIsDown(BvKey::kUp))
	{
		m_ZPos += m_Dt * 4.0f;
	}
	else if (m_Keyboard.KeyIsDown(BvKey::kDown))
	{
		m_ZPos -= m_Dt * 4.0f;
	}
...
void TextureArray::OnUpdateUI()
{
	BeginDrawDefaultUI();
	ImGui::SliderInt("Texture Index", &m_CurrArrayIndex, 0, m_MaxArrayIndex);
	EndDrawDefaultUI();
}


void TextureArray::OnRender()
{
	auto width = m_pWindow->GetWidth();
	auto height = m_pWindow->GetHeight();
...
void TextureArray::CreateRenderTargets()
{
