...
SkyBoxVSOutput main(SkyBoxVSInput input)
{
    ...
    float4 clipPos = mul(float4(input.inPos, 1.0f), wvp);
...
TextureCubeArray samplerTexture : register(t1);
...
float4 main(SkyBoxVSOutput input) : SV_Target
{
    ...
    return samplerTexture.Sample(samplerObj, float4(input.outTexCoords, arrayIndex));
}
)raw";
