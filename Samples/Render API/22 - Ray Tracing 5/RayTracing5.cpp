...
void RayTracing5::OnUpdate()
{
	...
	XMStoreFloat3x4(reinterpret_cast<XMFLOAT3X4*>(m_Instances[2].m_Transform), m_WorldPos[2]);
	...
	XMStoreFloat4x4(&m_pHitData->world[2], m_WorldPos[2]);
...
{
	{
		...
		pipelineDesc.m_MaxPayloadSize = 64;
...
void RayTracing5::CreateBLAS()
{
	BvRCRef<IBvAccelerationStructure> blas[2];
	{
		...
		m_Device->CreateAccelerationStructure(blasDesc, &blas[0]);
	}

	{
		...
		geomDesc.m_Triangle.m_IndexFormat = IndexFormat::kU32;
		m_Device->CreateAccelerationStructure(blasDesc, &blas[1]);
	}
	...
	BvRCRef<IBvBuffer> compactBuffer;
	m_Device->CreateBuffer(bfd, &compactBuffer);

	RayTracingAccelerationStructurePostBuildDesc postBuild;
	postBuild.m_Type = RayTracingAccelerationStructurePostBuildType::kCompactedSize;
	postBuild.m_pDstBuffer = compactBuffer;

	m_Context->NewCommandList();
	m_Context->BuildRayTracingAccelerationStructures(2, buildDescs, &postBuild);
	...
	BvConsole::Printf("Size Diff: %llu | %llu\n", blas[0]->GetBuildSizes().m_Build, compactedSizes[0]);
	BvConsole::Printf("Size Diff: %llu | %llu\n", blas[1]->GetBuildSizes().m_Build, compactedSizes[1]);

	{
		...
		RayTracingAccelerationStructureCopyDesc copy;
		copy.m_CopyMode = RayTracingAccelerationStructureCopyMode::kCompact;
		copy.m_pSrc = blas[0];
		copy.m_pDst = m_BLAS[0];
		m_Context->CopyRayTracingAccelerationStructure(copy);
...
void RayTracing5::CreateTLAS()
{
	...
	instanceDesc.m_Instance.m_InstanceCount = 3;
	...
	bufferDesc.m_Size = sizeof(RayTracingAccelerationStructureInstanceDesc) * 3;
	bufferDesc.m_MemoryType = MemoryType::kUpload;
	m_Device->CreateBuffer(bufferDesc, &m_StagingBuffer);

	for (auto i = 0; i < 3; i++)
	{
		m_Instances[i].m_AccelerationStructure = m_BLAS[u32(i != 0) * 1]->GetDeviceAddress();
	...
	m_TLAS->WriteTopLevelInstances(m_StagingBuffer, 3, m_Instances);
	...
	tlasBuildGeomDesc.m_Instance.m_Count = 3;
