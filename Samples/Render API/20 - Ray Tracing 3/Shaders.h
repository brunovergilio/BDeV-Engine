...
// ByteAddressBuffer allows arbitrary stride/offset unpacking of interleaved data
...
void main(inout RayPayload payload, in BuiltInTriangleIntersectionAttributes attribs)
{
    ...
    // Interpolate vertex normals using barycentric weights
    float3 normal = normalize(v0.normal * barycentrics.x + v1.normal * barycentrics.y + v2.normal * barycentrics.z);
	
    // Transform normal to world space (using reverse multiplication order for row-major matrix)
    normal = normalize(mul(float4(normal, 0.0f), modelMatrix).xyz);
	
    // Basic Lambertian diffuse with 0.1 ambient floor
    float NdotL = max(dot(normal, -lightDir), 0.1f);
    
    payload.color = float4(1.0f, 1.0f, 1.0f, 1.0f) * NdotL;
}
)raw";
