...
public:
	...
	ReverseIterator rend() const { return ReverseIterator(m_pData + m_Size); }
...
template<class Type>
inline BvVector<Type>::BvVector(const size_t capacity)
{
	Grow(capacity);
}
...
template<class Type>
inline BvVector<Type>::BvVector(BvVector && rhs)
{
	*this = std::move(rhs);
}
...
template<class Type>
inline BvVector<Type>& BvVector<Type>::operator=(BvVector && rhs)
{
	if (this != &rhs)
	{
		m_pData = rhs.m_pData;
...
inline void BvVector<Type>::Grow(const size_t size)
{
	...
	void * pNewData = BvAlloc(sizeof(Type) * m_Capacity);
	if (m_Size > 0)
	{
		memcpy(pNewData, m_pData, sizeof(Type) * m_Size);
	}
	BvFree(m_pData);
