...
struct ShaderResourceLayoutDesc
{
	...
	void AddResource(const char* const pName, const ShaderResourceType resourceType, const ShaderStage shaderStages,
	...
	void AddStaticSampler(const char* const pName, BvSampler ** ppSamplers, const ShaderStage shaderStages,
		const u32 binding, const u32 set = 0, const u32 count = 1)
	{
		auto result = m_PerSetData.Emplace(set);
		if (result.second)
	...
	void AddPushConstant(const char* const pName, const ShaderStage shaderStages, const u32 size, const u32 binding = 0)
	{
