...
BvKeyboardWindows::BvKeyboardWindows()
{
	RAWINPUTDEVICE rawInputDevice{};
	rawInputDevice.usUsagePage = 0x1;
	rawInputDevice.usUsage = 0x6;
...
bool BvKeyboardWindows::KeyWentDown(BvKey key) const
{
...
bool BvKeyboardWindows::KeyWentUp(BvKey key) const
{
	return (g_CurrGlobalKeyStates[u64(key) >> kDivShift] & (1ull << u64(u64(key) & kU64Mask))) == 0;
}


bool BvKeyboardWindows::KeyIsPressed(BvKey key) const
{
	return (g_CurrGlobalKeyStates[u64(key) >> kDivShift] & (1ull << u64(u64(key) & kU64Mask))) != 0;
}


void BvKeyboardWindows::SetGlobalKeyboardStateData(BvKey key, u32 scanCode, bool isKeyDown) const
{
...
void BvKeyboardWindows::ProcessRawInputKeyboardMessage(const RAWKEYBOARD& rawKB) const
{
