...
class IBvRenderEngine
{
	BV_NOCOPYMOVE(IBvRenderEngine);

public:
	virtual const BvVector<BvGPUInfo>& GetGPUs() const = 0;
	template<typename T = IBvRenderDevice> BV_INLINE T* CreateRenderDevice(const BvRenderDeviceCreateDesc& deviceCreateDesc) { return static_cast<T*>(CreateRenderDeviceImpl(deviceCreateDesc)); }
