...
VkBufferUsageFlags GetVkBufferUsageFlags(const BufferUsage usageFlags, bool formatted)
{
	...
	if (EHasFlag(usageFlags, BufferUsage::kRayTracing			))
	{
		bufferUsageFlags |= VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR
			...
			/*| ((bufferUsageFlags & VK_BUFFER_USAGE_STORAGE_BUFFER_BIT) ? VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR : 0)*/ | VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR;
	}
...
VkAccessFlags2 GetVkAccessFlags(const ResourceAccess resourceAccess)
{
	...
	if (EHasFlag(resourceAccess, ResourceAccess::kAccelerationStructureWrite)) { accessFlags |= VK_ACCESS_2_ACCELERATION_STRUCTURE_WRITE_BIT_KHR; }
...
VkAccessFlags2 GetVkAccessFlags(const ResourceState resourceState, bool rayTracing)
{
	...
	case ResourceState::kPixelShaderResource:	return VK_ACCESS_2_SHADER_READ_BIT | VK_ACCESS_2_SHADER_STORAGE_READ_BIT;
	...
	case ResourceState::kASBuildWrite:		return VK_ACCESS_2_ACCELERATION_STRUCTURE_WRITE_BIT_KHR;
	case ResourceState::kRayTracing:		return VK_ACCESS_2_ACCELERATION_STRUCTURE_READ_BIT_KHR;
	}
...
VkPipelineStageFlags2 GetVkPipelineStageFlags(const VkAccessFlags2 accessFlags, bool geom, bool tess, bool mesh, bool rt, bool rt1)
{
	...
	if (accessFlags & (VK_ACCESS_2_UNIFORM_READ_BIT | VK_ACCESS_2_SHADER_READ_BIT | VK_ACCESS_2_SHADER_WRITE_BIT))
	{
		stageFlags |= VK_PIPELINE_STAGE_2_VERTEX_SHADER_BIT
			| (tess ? VK_PIPELINE_STAGE_2_TESSELLATION_CONTROL_SHADER_BIT | VK_PIPELINE_STAGE_2_TESSELLATION_EVALUATION_SHADER_BIT : 0)
			| (geom ? VK_PIPELINE_STAGE_2_GEOMETRY_SHADER_BIT : 0)
			| VK_PIPELINE_STAGE_2_FRAGMENT_SHADER_BIT
			| VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT
			| (mesh ? VK_PIPELINE_STAGE_2_MESH_SHADER_BIT_EXT | VK_PIPELINE_STAGE_2_TASK_SHADER_BIT_EXT : 0)
			| (rt ? VK_PIPELINE_STAGE_2_RAY_TRACING_SHADER_BIT_KHR : 0);
	}
	...
	if (accessFlags & (VK_ACCESS_2_ACCELERATION_STRUCTURE_READ_BIT_KHR | VK_ACCESS_2_ACCELERATION_STRUCTURE_WRITE_BIT_KHR))
	{
		stageFlags |= VK_PIPELINE_STAGE_2_ACCELERATION_STRUCTURE_BUILD_BIT_KHR | VK_PIPELINE_STAGE_2_RAY_TRACING_SHADER_BIT_KHR
			| (rt1 ? VK_PIPELINE_STAGE_2_ACCELERATION_STRUCTURE_COPY_BIT_KHR : 0);
	}
...
{
	auto psoFlags = VK_QUERY_PIPELINE_STATISTIC_INPUT_ASSEMBLY_VERTICES_BIT
	...
	auto meshFlags = VK_QUERY_PIPELINE_STATISTIC_TASK_SHADER_INVOCATIONS_BIT_EXT | VK_QUERY_PIPELINE_STATISTIC_MESH_SHADER_INVOCATIONS_BIT_EXT;

	return (!meshShader ? psoFlags : meshFlags) | VK_QUERY_PIPELINE_STATISTIC_FRAGMENT_SHADER_INVOCATIONS_BIT;
}
