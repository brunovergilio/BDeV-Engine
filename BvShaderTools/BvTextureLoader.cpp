...
#undef ISBITMASK
	...
	LoaderResult FillInitData(size_t width,
		...
		size_t& tdepth,
		size_t& skipMip,
		BvVector<SUBRESOURCE_DATA>& initData)
	{
		if (!bitData)
		{
			return LoaderResult::kBadPointer;
			...
			for (size_t j = 0; j < arraySize; j++)
			{
				...
				for (size_t i = 0; i < mipCount; i++)
				{
					...
					if ((mipCount <= 1) || !maxsize || (w <= maxsize && h <= maxsize && d <= maxsize))
					{
						...
						SUBRESOURCE_DATA res =
						{
							pSrcBits,
							static_cast<int64_t>(RowBytes),
							static_cast<int64_t>(NumBytes)
						};
	...
	LoaderResult CreateTextureFromDDS(const DDS_HEADER* header,
		...
		BvVector<SUBRESOURCE_DATA>& subresources,
...
LoaderResult DirectX::LoadDDSTextureFromMemory(
...
_Use_decl_annotations_
LoaderResult DirectX::LoadDDSTextureFromMemoryEx(
	ID3D12Device* d3dDevice,
	const uint8_t* ddsData,
	size_t ddsDataSize,
	size_t maxsize,
...
//--------------------------------------------------------------------------------------
_Use_decl_annotations_
LoaderResult DirectX::LoadDDSTextureFromFile(
	ID3D12Device* d3dDevice,
...
_Use_decl_annotations_
LoaderResult DirectX::LoadDDSTextureFromFileEx(
	ID3D12Device* d3dDevice,
	const wchar_t* fileName,
	size_t maxsize,
	D3D12_RESOURCE_FLAGS resFlags,
	...
	bool* isCubeMap)
{
	...
	LoaderResult hr = LoadTextureDataFromFile(fileName,
		ddsData,
		&header,
		&bitData,
		&bitSize
